...
```
[로컬 파일 수정] → debounce 대기 → rsync push → 서버에 반영
[서버 파일 수정] → 폴링 감지     → rsync pull → 로컬에 반영
[수동 / 주기적]  → 전체 비교     → rsync 벌크 동기화 (첫 동기화도 빠름)
```
...
| Push debounce | 로컬 변경 후 push까지 대기 시간 (초) | `5` |
| Pull interval | 서버 변경 확인 폴링 주기 (초) | `30` |
